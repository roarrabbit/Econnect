...
def Econ_connect(ip, user, pwd, secret):
    ...
    try:
        ...
        # 有<说明为华为交换机
        if len(secret) != 0:
            print('已连接 %s  进入特权模式...' % sshconfirm, end=' ')
            stdout.flush()
            # 进入特权模式
            net_connect.enable()
            stdout.flush()
            ...
            print('已连接 %s ...' % sshconfirm, end=' ')
        return net_connect, sshconfirm
    except:
        net_connect.disconnect()
        print('进入特权模式失败(,,• ₃ •,,)...请相关配置！')
        return 0, sshconfirm
...
def Econ_inspection(net_connect, sshconfirm):
    ...
    if '<' not in sshconfirm:
        ...
        output_cpu = findall(r': (\d.*?)%', output_cpu)
    print('CPU使用率： 五秒内使用：{0}%， 一分钟内使用：{1}%， 五分钟内使用{2}%\n'.format(output_cpu[0], output_cpu[1], output_cpu[2]))
    # print('断开连接中...', end='')
    net_connect.disconnect()
    # print('已断开\n')
            ...
            for i in Switch_list_choice:
                ...
                if net_connect == 0:
                    ...
                    if sshconfirm == 0:
                        ...
                        Econ_inspection_box.append([i[0], '1connect_faild', -1, -1, -1, -1])
                        continue
                    else:
                        # 如果sshconfirm=0则说明连接成功但是无法进入特权模式
                        Econ_inspection_box.append([i[0], sshconfirm, -1, -1, -1, -1])
                        continue
                # 执行巡检动作
                ...
                # 由于没有使用次数统计所以用总数-1代替
                Econ_inspection_box[len(Econ_inspection_box) - 1].insert(0, i[0])
